...
class TetrisApp(object):
    ...
    def init_game(self):
        ...
        #pygame.time.set_timer(pygame.USEREVENT + 1, 100)   #timer go to game speed
    ...
    def add_cl_lines(self, n,floor):
        ...
        #linescores = [0, 0, 3, 6, 10]
        floorscores =np.linspace(1,3,13)
        floorscores=np.insert(floorscores,0,0)
        #floorscores = [0, 1, 1.17, 1.33, 1.5, 1.7, 1.8, 2, 2.17, 2.3, 2.5, 2.67, 2.8, 3]
        #floorscores = [0,1, 1.0833, 1.1667, 1.25, 1.3333, 1.4167, 1.5, 1.5833, 1.6667, 1.75, 1.8333, 1.9167, 2]
        self.lines += n
        #self.score += linescores[n]
        global game_score
        #game_score += linescores[n]
        self.score +=floorscores[floor]*self.level
    ...
    def drop(self, manual):
        new_y = self.stone_y
        if not self.gameover :
            #self.score += 0.01 if manual else 0           #drop score
    ...
    def step(self, action):
        ...
        pygame.draw.line(self.screen, (255, 0, 0), (0, 0), (self.width / 2 + 18, 0), boundary_size )
        pygame.draw.line(self.screen, (255, 255, 255), (self.width / 2 + 18, 0), (self.width, 0), boundary_size)
        pygame.draw.line(self.screen, (255, 0, 0), (0, self.height - 1), (self.width / 2 + 18, self.height - 1),
                         boundary_size)
        pygame.draw.line(self.screen, (255, 255, 255), (self.width / 2 + 18, self.height - 1),
...
Press space to continue""" % self.score)
                ...
                if self.paused:
                    ...
                    pygame.draw.line(self.screen, (255, 255, 255), (self.width + boundary_size, 0),
                                     (self.width + boundary_size, self.height - 1), boundary_size)
                    pygame.draw.line(self.screen, (255, 255, 255), (0, 0), (self.width/2+18, 0), boundary_size)
                    pygame.draw.line(self.screen, (255, 255, 255), (self.width / 2 + 18, 0), (self.width, 0), boundary_size)
                    pygame.draw.line(self.screen, (255, 255, 255), (0, self.height - 1), (self.width / 2 + 18, self.height - 1),
                                     boundary_size)
                    pygame.draw.line(self.screen, (255, 255, 255), (self.width / 2 + 18, self.height - 1),(self.width, self.height - 1),
                                     boundary_size)
