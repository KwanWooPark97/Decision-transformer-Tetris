...
class trainer():
    def __init__(self):
        ...
        self.model.load_state_dict(torch.load('save_model.pt')) #학습한 모델을 불러옵니다.
    ...
    def test(self):
        ...
        for e in range(EPISODES):
            ...
            while not done and time_step <= 1000:
                ...
                #print(torch.Tensor(np.array(reward_deq)).to(device).unsqueeze(dim=0).unsqueeze(dim=-1).shape)
                #exit(1)
                time.sleep(0.2)
                global_step += 1
                ...
                #print(action)
                act = action_preds_act[-1].cpu().detach().numpy() #deque는 numpy를 지원해서 tensor를 numpy로 변환해야합니다
                #print(act.type)
